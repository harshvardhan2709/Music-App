...
export default function PlaylistDetailScreen() {
    ...
    return (
        <View className="flex-1 bg-white dark:bg-black p-5 pt-14">
            {/* Header */}
            <View className="flex-row items-center justify-between mb-5 bg-gray-200 dark:bg-gray-900 p-4 rounded-3xl">
                <TouchableOpacity onPress={() => router.back()} className="p-2">
                    <FontAwesome name="arrow-left" size={20} color={colorScheme === 'dark' ? 'white' : 'black'} />
                </TouchableOpacity>
                ...
                {isEditing ? (
                    <TextInput
                        ...
                        className="flex-1 mx-4 text-xl font-bold border-b border-gray-300 dark:border-gray-700 text-black dark:text-white"
                        autoFocus
                        onBlur={handleRename}
                        onSubmitEditing={handleRename}
                    />
                ) : (
                    <Text className="text-2xl font-bold flex-1 mx-4 text-center text-black dark:text-white" numberOfLines={1}>
                        {playlist.name}
                    </Text>
                )}
                ...
                <View className="flex-row gap-4">
                    <TouchableOpacity onPress={() => setIsEditing(!isEditing)}>
                        <FontAwesome name="pencil" size={20} color={isEditing ? "blue" : (colorScheme === 'dark' ? "white" : "black")} />
                    </TouchableOpacity>
                ...
                renderItem={({ item }) => (
                    <View className="flex-row items-center justify-between border-b border-[#eee] dark:border-gray-800 py-3">
                        <TouchableOpacity
                            ...
                            onPress={() => play(item)}
                        >
                            <Text className="text-base font-semibold text-black dark:text-white" numberOfLines={1}>{item.filename}</Text>
